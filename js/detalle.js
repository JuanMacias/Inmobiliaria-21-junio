// La función initMapDetalle debe estar fuera del addEventListener para ser accesible
// por el callback de la API de Google Maps.
function initMapDetalle(idPropiedad) {
  fetch("./propiedades.json")
    .then(res => res.json())
    .then(data => {
      const propiedad = data.find(p => p.id === idPropiedad);
      if (propiedad && propiedad.lat && propiedad.lng) {
        const coordenadas = { lat: propiedad.lat, lng: propiedad.lng };
        ...
        const contenidoGlobo = `
          <div class="info-window-custom">
            <img src="${propiedad.imagen}" alt="${propiedad.titulo}" class="info-window-img" />
        ...
        // Abre el globo automáticamente al cargar el mapa
        infoWindow.open({
            anchor: marker,
            map,
            shouldFocus: false,
        ...
        console.error("No se encontraron coordenadas para esta propiedad.");
      }
    })
  ...
  function renderizarDetalle(prop) {
    ...
    contenedorDetalle.innerHTML = `
      ...
      <div class="info-bloque mapa">
        <h4>Ubicación</h4>
        <div id="mapa-detalle-propiedad"></div>
      </div>
    `;
    ...
    // Llamada a la función para inicializar el mapa
    // Esto se ejecutará solo cuando el resto del contenido de la página esté cargado
    if (typeof initMapDetalle === 'function') {
      initMapDetalle(prop.id);
    }
  }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("./propiedades.json")
        .then(res => res.json())
        .then(data => {
            const prop = data.find(p => p.id === idPropiedad);
    ...
    function renderizarDetalle(prop) {
        const miniaturasHtml = (prop.imagenes || [prop.imagen]).map((img, i) => `
            <img src="${img}" class="miniatura ${i === 0 ? 'activa' : ''}" data-index="${i}" alt="Miniatura ${i + 1}" />
        `).join("");
        ...
        contenedorDetalle.innerHTML = `
            ...
            <div class="galeria">
                <div class="galeria-principal">
                    <button class="flecha izquierda" id="btn-prev-galeria">❮</button>
                    <img id="imagen-activa" src="${prop.imagenes?.[0] || prop.imagen}" alt="Imagen principal" />
                    <button class="flecha derecha" id="btn-next-galeria">❯</button>
                </div>
                <div class="galeria-miniaturas">
                    ${miniaturasHtml}
        ...
        setTimeout(() => {
            if (typeof google !== 'undefined' && typeof google.maps !== 'undefined') {
                ...
                const contenidoGlobo = `
                    <div class="info-window-custom">
                        <h4>${prop.titulo}</h4>
                        <img src="${prop.imagen}" alt="Imagen de la propiedad" class="info-window-img">
                        <p><strong>Precio:</strong> u$s ${prop.precio || '-'}</p>
                ...
                console.error("Error: Google Maps API no se ha cargado.");
            }
        }, 500);
    }
});